...
class faceRecognition:
    def __init__(self):
        ...
        self.face_list_id ="face_list_id3"
        self.azureFaceList = AzureFaceList(self.face_list_id, AZURE_KEY, AZURE_ENDPOINT)
        
        # Creating supabase client
        url = os.getenv("SUPABASE_AZURE_URL")
        key = os.getenv("SUPABASE_AZURE_KEY")
    ...
    def findFace(self, face, frame):
        ...
        time1 = time.time()
        fullBucket = self.supabase.storage.from_('bucketTest').list()
        bucketSize = len(fullBucket)
        time2 = time.time()
        print(f"Time to retrieve size: {time2-time1}")
        print(f"Size of bucket: {bucketSize}")
        
        time1 = time.time()
        face_jpg = cv2.imencode(".jpg", face[1])[1].tobytes()
        destination = f"image_{bucketSize}.jpg"
